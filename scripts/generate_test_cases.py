...
def make_case(case_id, scenario, query, relevant_docs, reference_answer, relevant_articles=None):
    relevant_ids = []
    ...
    if relevant_articles:
        payload["relevant_articles"] = [str(x).strip() for x in relevant_articles if str(x).strip()]
    return payload


def add_case(cases, cid, scenario, query, docs, answer, relevant_articles=None):
    case = make_case(cid, scenario, query, docs, answer, relevant_articles=relevant_articles)
    if case:
        cases.append(case)
        return cid + 1
    return cid
...
def main():
    ...
    for n in article_numbers:
        cid = add_case(cases, cid, "exact-keyword-fact", f"Điều {n} Luật Hôn nhân và gia đình 2014 quy định gì?", law_2014, f"Điều {n} quy định nội dung tương ứng trong Luật Hôn nhân và gia đình 2014.", relevant_articles=[str(n)])
