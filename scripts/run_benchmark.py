...
def main():
    ...
    parser.add_argument("--testset", default=str(ROOT_DIR / "data" / "evaluation" / "legal_test_cases.json"))
    ...
    for strategy in args.strategies:
        ...
        for scenario, metrics in result["breakdown"].items():
            if scenario == "global_overall":
                continue
            print(f"  {scenario}: Hit={metrics.get('hit_at_k',0):.3f} | P={metrics.get('precision_at_k',0):.3f} | R={metrics.get('recall_at_k',0):.3f} | MRR={metrics.get('reciprocal_rank',0):.3f}")
