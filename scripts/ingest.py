...
from src.ingestion.loader import load_documents_by_source
...
def main():
    source = str(getattr(config.dataset, "source", "huggingface"))
    print(f"🚀 Loading Vietnamese legal documents from source={source}...")
    docs = load_documents_by_source(config, sample_size=getattr(config.dataset, "sample_size", None))
    if not docs:
    ...
    if str(getattr(config.dataset, "source", "huggingface")) == "local_pdf":
        print("Tip: for full local-PDF processed outputs, use: python scripts/ingest_family_law_pdfs.py")
