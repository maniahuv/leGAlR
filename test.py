...
if __name__ == "__main__":
    question = os.getenv(
        "TEST_QUESTION",
        "Tòa án giải quyết thuận tình ly hôn khi vợ chồng đáp ứng những điều kiện nào?",
    )

    # Current benchmark shows dense is the fastest and safest non-graph default.
    # Use TEST_STRATEGY=hybrid_rerank/auto/graph to compare strategies.
    strategy = os.getenv("TEST_STRATEGY", "dense")
    k = int(os.getenv("TEST_K", "3"))
    num_runs = int(os.getenv("TEST_RUNS", "5"))
    max_context_chars = int(os.getenv("TEST_MAX_CONTEXT_CHARS", "3000"))
    enable_llm = os.getenv("TEST_ENABLE_LLM", "0").strip().lower() in {"1", "true", "yes"}
    ...
    warmup_prompt = generate_answer_tool.invoke({"query": question, "context": warmup_context})
    ...
    for i in range(num_runs):
        ...
        t0 = time.perf_counter()
        prompt = generate_answer_tool.invoke({"query": question, "context": context})
