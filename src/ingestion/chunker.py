...
def _split_by_article(doc: Document, config) -> list[Document]:
    ...
    for article in articles:
        ...
        if getattr(article, "chapter", ""):
            article_meta["chapter"] = article.chapter
        if getattr(article, "section", ""):
            article_meta["section"] = article.section
        ...
        if len(article.text) > max_len:
            ...
            for split_part in split_parts:
                ...
                if getattr(article, "chapter", ""):
                    split_meta.setdefault("chapter", article.chapter)
                if getattr(article, "section", ""):
                    split_meta.setdefault("section", article.section)
...
def _is_low_value_chunk(doc: Document) -> bool:
    """Drop source-header-only chunks that pollute retrieval.

    They often contain only title/number/effective-status metadata and no legal
    norm. Keeping the same information in metadata is enough; indexing such
    chunks causes dense/BM25 to return context that cannot answer questions.
    """
    text = (doc.page_content or "").strip()
    if not text:
        return True
    if len(text) < 120 and "Điều " not in text:
        return True
    if text.startswith("Luật ") and "Điều " not in text and len(text) < 180:
        return True
    if text.startswith("Nghị định ") and "Điều " not in text and len(text) < 180:
        return True
    if text.startswith("Thông tư") and "Điều " not in text and len(text) < 180:
        return True
    return False
...
def _add_chunk_metadata(chunks: list[Document]) -> list[Document]:
    ...
    for chunk in chunks:
        ...
        # Keep hierarchy metadata when available from legal_parser.
        if meta.get("chapter"):
            meta["chapter"] = str(meta.get("chapter", "")).strip()
        if meta.get("section"):
            meta["section"] = str(meta.get("section", "")).strip()
        if clause:
            meta["clause"] = clause
        chunk.metadata = meta
    return chunks
...
def chunk_documents(docs: list[Document], config) -> list[Document]:
    ...
    chunks = [chunk for chunk in chunks if not _is_low_value_chunk(chunk)]
