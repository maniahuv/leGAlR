...
from src.ingestion.pdf_loader import load_local_family_law_documents
...
def load_documents_by_source(config, sample_size: int | None = None) -> list[Document]:
    """Load documents from the configured corpus source.

    Supported sources:
    - huggingface: th1nhng0/vietnamese-legal-documents, filtered to family-law.
    - local_pdf: official PDFs listed in data/raw/family_law/manifest.jsonl.
    """
    source = str(getattr(config.dataset, "source", "huggingface")).lower().strip()
    if source == "local_pdf":
        return load_local_family_law_documents(config)
...
def load_document(config, sample_size=None):
    ...
    return load_documents_by_source(config, sample_size=sample_size)
