...
from src.retrieval.legal_authorities import (
    assemble_context_docs,
    authority_candidates,
    dedupe_documents,
    detect_authority_intent,
    hierarchy_label,
    context_coverage,
    ensure_authority_coverage,
)
...
def is_relation_query(query: str) -> bool:
    ...
    Lưu ý AUTO_V5: nhiều câu hộ tịch có từ "bổ sung", "thay đổi",
    "cải chính" nhưng không hỏi quan hệ sửa đổi/bổ sung giữa văn bản.
    Các câu đó phải đi Hybrid/Authority-Hybrid, không đi Graph.
    """
    q = (query or "").lower().strip()
    if not q:
        return False

    civil_status_false_positive_patterns: tuple[str, ...] = (
        r"\bbổ\s+sung\s+(tên|thông\s+tin|hộ\s+tịch|giấy\s+khai\s+sinh|cha|mẹ|con)\b",
        r"\bthay\s+đổi\s+(họ|tên|họ\s+tên|hộ\s+tịch|giấy\s+khai\s+sinh)\b",
        r"\bcải\s+chính\s+hộ\s+tịch\b",
        r"\bcập\s+nhật\s+(tên|thông\s+tin|hộ\s+tịch)\b",
        r"\bđổi\s+họ\s+cho\s+con\b",
        r"\bđổi\s+tên\s+cho\s+con\b",
    )
    if any(re.search(pattern, q) for pattern in civil_status_false_positive_patterns):
        return False
...
def format_docs_for_context(docs: list[Document]) -> str:
    ...
    coverage = context_coverage(docs, getattr(format_docs_for_context, "_last_query", "")) if getattr(format_docs_for_context, "_last_query", "") else {}
    coverage_note = ""
    if coverage and coverage.get("required_pairs"):
        missing = coverage.get("missing_pairs") or []
        if missing:
            def _fmt_missing(pair):
                d, a = pair
                return f"{d}" if a == "__full_doc__" else f"{d} Điều {a}"
            coverage_note = "[Kiểm tra căn cứ] Còn thiếu căn cứ bắt buộc trong context: " + ", ".join(_fmt_missing(pair) for pair in missing) + "\n\n"
        else:
            coverage_note = "[Kiểm tra căn cứ] Context đã bao phủ các căn cứ bắt buộc được nhận diện.\n\n"
    ...
    for i, doc in enumerate(docs, start=1):
        ...
        hierarchy = str(meta.get("hierarchy_label") or hierarchy_label(doc) or "").strip()
        hierarchy_note = f"\nCấu trúc pháp lý: {hierarchy}" if hierarchy else ""
        graph_path = meta.get("graph_path")
        graph_note = f"\nQuan hệ đồ thị: {graph_path}" if graph_path else ""
        blocks.append(
            f"[Nguồn {i}] {citation}{hierarchy_note}{graph_note}\n{(doc.page_content or '')[:max_chars]}"
        )
...
def format_docs_for_context_with_query(docs: list[Document], query: str) -> str:
    old = getattr(format_docs_for_context, "_last_query", "")
    try:
        setattr(format_docs_for_context, "_last_query", query or "")
        return format_docs_for_context(docs)
    finally:
        setattr(format_docs_for_context, "_last_query", old)
...
def _authority_dense_search(store, bm25, query: str, k: int = 5) -> list[Document]:
    """Dense baseline with deterministic authority override for high-confidence intents.

    The previous upgrade routed most normal questions to HYBRID_RERANK and
    caused regression. This route keeps dense as the stable recall backbone, then
    injects only a small number of mandatory legal-authority candidates for
    clearly detected intents.
    """
    authority_k = int(getattr(config.retrieval, "authority_k", 20))
    authority_docs = authority_candidates(bm25, query, k=authority_k)
    dense_docs = dense_search(store, query, k=max(k * 4, 20))

    # Put authorities first, then let the existing legal reranker decide.
    candidates = dedupe_documents(authority_docs + dense_docs)
    ranked = rerank(query, candidates, k=max(k, 5), force=True)
    ranked = ensure_authority_coverage(bm25, ranked, query, max_docs=k)

    return assemble_context_docs(
        bm25,
        ranked,
        query,
        max_docs=k,
        max_article_chars=int(getattr(config.retrieval, "max_article_chars", 6500)),
    )


def _authority_hybrid_search(store, bm25, query: str, k: int = 5) -> list[Document]:
    """Hybrid backbone with deterministic authority override for AUTO_V5.

    Benchmark forum 300 câu cho thấy HYBRID ổn định hơn DENSE trên câu hỏi
    thực tế, đặc biệt là nhóm thủ tục và đa căn cứ. Hàm này giữ HYBRID làm
    backbone, sau đó chèn authority candidates cho các intent có căn cứ rõ.

    Không gọi rerank(force=True) ở đây để tránh biến AUTO thành
    HYBRID_RERANK chậm. Rerank vẫn có thể được benchmark riêng bằng strategy
    ``hybrid_rerank``.
    """
    authority_k = int(getattr(config.retrieval, "authority_k", 20))
    authority_docs = authority_candidates(bm25, query, k=authority_k)
    hybrid_docs = hybrid_search(store, bm25, query, k=max(k * 4, 20))

    candidates = dedupe_documents(authority_docs + hybrid_docs)
    docs = ensure_authority_coverage(bm25, candidates, query, max_docs=k)
...
def retrieve_documents(query: str, k: int = 5, strategy: str = "auto") -> list[Document]:
    ...
    - authority_dense: dense baseline + deterministic authority candidates.
    - authority_hybrid: hybrid backbone + deterministic authority candidates.
    - auto:
        + graph for clear relation/document-link queries.
        + authority_hybrid for high-confidence legal-authority intents.
        + hybrid for remaining normal legal QA.

      AUTO_V5 uses HYBRID as the default backbone because the 300-question
      forum benchmark showed HYBRID is stronger than DENSE on real-world
      procedural and multi-authority questions. Rerank is not enabled by default
    ...
    if strategy == "dense":
    ...
    elif strategy == "authority_dense":
        docs = _authority_dense_search(store, bm25, query, k=k)
        route = "authority_dense"

    elif strategy == "authority_hybrid":
        docs = _authority_hybrid_search(store, bm25, query, k=k)
        route = "authority_hybrid"
        ...
        # AUTO_V5:
        # - Relation/document-link queries go to Graph.
        # - Clear legal-authority intents go to Authority-Hybrid.
        # - All remaining real-world legal QA goes to Hybrid, not Dense.
        #
        # Reason: forum benchmark 300 câu cho thấy HYBRID đạt Hit@5 và
        # ArticleHit@5 tốt hơn DENSE, còn HYBRID_RERANK quá chậm để bật
        # mặc định trong AUTO.
        ...
        elif detect_authority_intent(query) is not None:
            route = "authority_hybrid"
            docs = _authority_hybrid_search(store, bm25, query, k=k)
        else:
            route = "hybrid"
    ...
    if route in {"hybrid", "hybrid_rerank", "authority_dense", "authority_hybrid"}:
        docs = ensure_authority_coverage(bm25, docs, query, max_docs=k)
...
def retrieve_auto_tool(query: str, k: int = config.retrieval.k) -> list[dict]:
    """AUTO_V5: relation queries to graph, authority intents to authority_hybrid, and remaining questions to hybrid."""
...
9. Không cam kết chắc chắn kết quả xét xử/hành chính như "chắc chắn thắng", "chắc chắn được chia", "Tòa chắc chắn quyết định"; phải dùng cách diễn đạt phụ thuộc chứng cứ và thẩm quyền của Tòa án/cơ quan có thẩm quyền.
10. Nếu vấn đề phụ thuộc dữ kiện còn thiếu, nêu rõ giả định và dữ kiện cần bổ sung.
