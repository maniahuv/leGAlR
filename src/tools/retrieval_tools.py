...
def retrieve_documents(query: str, k: int = 5, strategy: str = "auto") -> list[Document]:
    ...
    - auto:
        + graph only for relation/document-link queries.
        + dense for normal legal QA on the expanded corpus.

      Hybrid_rerank is still available as an explicit strategy, but current
      benchmarks show Dense is the safest default non-graph route after adding
      civil status/adoption/domestic-violence/procedure/civil-law documents.
    """
...
def retrieve_auto_tool(query: str, k: int = config.retrieval.k) -> list[dict]:
    """Automatically route query to graph for relation queries, otherwise dense search."""
