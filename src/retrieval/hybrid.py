...
def hybrid_search(
    ...
    weights = [
        float(getattr(config.retrieval, "dense_weight", 0.75)),
        float(getattr(config.retrieval, "bm25_weight", 0.20)),
    ]
    if meta_docs:
        ranked_lists.append(meta_docs)
        weights.append(float(getattr(config.retrieval, "metadata_weight", 0.10)))
    ...
    # Dense is the most reliable baseline on the expanded family-law corpus.
    # Preserve a small number of top Dense candidates so BM25 noise cannot
    # completely displace semantically strong matches. This makes HYBRID a
    # dense-preserving hybrid rather than a BM25-dominated fusion.
    preserve_dense_top = int(getattr(config.retrieval, "hybrid_preserve_dense_top_k", 2))
