...
from src.retrieval.legal_authorities import authority_candidates, detect_authority_intent, expand_legal_query
...
def reciprocal_rank_fusion(
    ...
    fusion_metadata_weight = float(getattr(config.retrieval, "fusion_metadata_weight", 0.06))
...
def hybrid_search(
    ...
    """Legal-aware hybrid search.

    This remains a HYBRID retrieval method, not a replacement for dense.py.
    Dense/BM25/metadata still provide recall, but deterministic legal-authority
    candidates are injected into the pool so obvious legal authorities do not
    disappear before reranking.
    """
    ...
    expanded_query = expand_legal_query(query) if intent else query

    dense_docs = dense_search(store, query, k=dense_k)
    bm25_docs = bm25_search(bm25, expanded_query, k=bm25_k)
    meta_docs = metadata_search(
        bm25,
        expanded_query,
        k=int(getattr(config.retrieval, "metadata_k", max(bm25_k, 30))),
    )
    auth_docs = authority_candidates(
        bm25,
        query,
        k=int(getattr(config.retrieval, "authority_k", 20)),
    ) if intent else []

    ranked_lists: list[list[Document]] = []
    weights: list[float] = []

    # Dense is the stable baseline in this corpus. Authority/BM25/metadata are
    # candidate channels, not hard replacements. Do not prepend authority docs in
    # HYBRID, otherwise broad queries regress by being dragged to the wrong law.
    ranked_lists.append(dense_docs)
    weights.append(float(getattr(config.retrieval, "dense_weight", 0.85)))

    ranked_lists.append(bm25_docs)
    weights.append(float(getattr(config.retrieval, "bm25_weight", 0.35)))

    if auth_docs:
        ranked_lists.append(auth_docs)
        weights.append(float(getattr(config.retrieval, "authority_weight", 0.55)))

    if meta_docs:
        ranked_lists.append(meta_docs)
        weights.append(float(getattr(config.retrieval, "metadata_weight", 0.12)))
    ...
    # Keep exact authority and metadata documents available in large rerank pools,
    # but never force them to the top of normal HYBRID results.
    fused = _dedupe_documents(fused + auth_docs + meta_docs)

    preserve_dense_top = int(getattr(config.retrieval, "hybrid_preserve_dense_top_k", 3))
    if preserve_dense_top > 0 and dense_docs:
        fused = _dedupe_documents(dense_docs[:preserve_dense_top] + fused)
